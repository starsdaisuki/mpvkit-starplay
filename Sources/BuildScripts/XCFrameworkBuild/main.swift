...
do {
    ...
    // Resume gate for StarPlay's local experiment. A documentation-only mpv failure must not
    // rebuild three already-good FFmpeg slices. Normal builds leave this unset and run everything.
    let onlyMPV = ProcessInfo.processInfo.environment["STARPLAY_ONLY_MPV"] == "1"
    let onlyCore = ProcessInfo.processInfo.environment["STARPLAY_ONLY_CORE"] == "1"
    ...
    if !onlyMPV {
        if !onlyCore {
            ...
            // StarPlay's SMB implementation is vendored in the app; this variant does not build Samba.
    ...
    var targets : [PackageTarget] {
        ...
        case .libmpv:
            return [
                .target(
                    name: "Libmpv",
                    url: "https://github.com/starsdaisuki/mpvkit-starplay/releases/download/\(BaseBuild.options.releaseVersion)/Libmpv.xcframework.zip",
                    checksum: ""
                ),
            ]
        case .FFmpeg:
            return  [
                .target(
                    name: "Libavcodec",
                    url: "https://github.com/starsdaisuki/mpvkit-starplay/releases/download/\(BaseBuild.options.releaseVersion)/Libavcodec.xcframework.zip",
                    checksum: ""
                ),
                .target(
                    name: "Libavdevice",
                    url: "https://github.com/starsdaisuki/mpvkit-starplay/releases/download/\(BaseBuild.options.releaseVersion)/Libavdevice.xcframework.zip",
                    checksum: ""
                ),
                .target(
                    name: "Libavformat",
                    url: "https://github.com/starsdaisuki/mpvkit-starplay/releases/download/\(BaseBuild.options.releaseVersion)/Libavformat.xcframework.zip",
                    checksum: ""
                ),
                .target(
                    name: "Libavfilter",
                    url: "https://github.com/starsdaisuki/mpvkit-starplay/releases/download/\(BaseBuild.options.releaseVersion)/Libavfilter.xcframework.zip",
                    checksum: ""
                ),
                .target(
                    name: "Libavutil",
                    url: "https://github.com/starsdaisuki/mpvkit-starplay/releases/download/\(BaseBuild.options.releaseVersion)/Libavutil.xcframework.zip",
                    checksum: ""
                ),
                .target(
                    name: "Libswresample",
                    url: "https://github.com/starsdaisuki/mpvkit-starplay/releases/download/\(BaseBuild.options.releaseVersion)/Libswresample.xcframework.zip",
                    checksum: ""
                ),
                .target(
                    name: "Libswscale",
                    url: "https://github.com/starsdaisuki/mpvkit-starplay/releases/download/\(BaseBuild.options.releaseVersion)/Libswscale.xcframework.zip",
                    checksum: ""
                ),
            ]
        case .openssl:
...
private class BuildMPV: BaseBuild {
    ...
    override func arguments(platform: PlatformType, arch: ArchType) -> [String] {
        var array = [
            ...
            "-Dmanpage-build=disabled", // libmpv xcframework 不需要 mpv.1；docutils 失败不能拖死库构建
...
private class BuildFFMPEG: BaseBuild {
    ...
    override func cFlags(platform: PlatformType, arch: ArchType) -> [String] {
        ...
        // `Library.openssl` 是下载包的逻辑名，真实 archive 是 libssl.a + libcrypto.a。
        // 不能交给 BaseBuild 的通用逻辑，否则它会机械生成不存在的 `-lopenssl`。
        let openssl = thinDir(library: .openssl, platform: platform, arch: arch)
        if FileManager.default.fileExists(atPath: openssl.path) {
            cFlags.append("-I\(openssl.path)/include")
        }
    ...
    override func ldFlags(platform: PlatformType, arch: ArchType) -> [String] {
        ...
        let openssl = thinDir(library: .openssl, platform: platform, arch: arch)
        if FileManager.default.fileExists(atPath: openssl.path) {
            ldFlags.append(contentsOf: ["-L\(openssl.path)/lib", "-lssl", "-lcrypto"])
        }
    ...
    override func arguments(platform: PlatformType, arch: ArchType) -> [String] {
        ...
        //        }
        var dependencyLibrary = [Library.openssl, .libfreetype, .libharfbuzz, .libfribidi, .libass, .vulkan, .libshaderc, .lcms2, .libplacebo, .libdav1d, .libuavs3d]
        if BaseBuild.options.enableGPL {
    ...
    private let ffmpegConfiguers = [
        ...
        // ⭐ StarPlay 的 App Store 变体必须留在 LGPL-2.1+：GMP/GnuTLS 已移除，
        // OpenSSL-only 的 LGPL 构建不需要 `--enable-version3`。FFmpeg 自己对该开关的
        // 定义就是 “upgrade (L)GPL to version 3”，留着会让删 GnuTLS 变成假修复。
        "--enable-optimizations", "--enable-pic", "--enable-runtime-cpudetect", "--enable-static", "--enable-thumb",
